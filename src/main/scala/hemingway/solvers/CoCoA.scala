package hemingway.solvers

import org.apache.spark.rdd.RDD
import hemingway.utils._
...
object CoCoA {
  ...
  def runCoCoA(
    ...
    val parts = data.partitions.length 	// number of partitions of the data, K in the paper
    ...
    var alpha = alphaVars.mapPartitions(x => Iterator.single(Vector(x.toArray)))
    var dataArr = data.mapPartitions(x => Iterator.single(x.toArray))
  ...
  def localSDCA(
    ...
    for (i <- 1 to localIters) {
      ...
      if (Math.abs(proj_grad) != 0.0 ) {
        val xnorm = Math.pow(breeze.linalg.norm(x), 2)
